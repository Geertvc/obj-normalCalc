...
public class CgPanel extends JPanel  implements ComponentListener {
	private static final long serialVersionUID = 7393873302756581573L;
	 ...
	 */ 
	public void drawPixel(int x, int y, float r, float g, float b) throws IllegalArgumentException {
		if (x>=0 && x<getWidth() && y>=0 && y<getHeight()) {
			rgbpixels[x+getWidth()*y] = 255<<24 | (int)(255*r)<<16 | (int)(255*g)<<8 | (int)(255*b);
		} else{
			throw new IllegalArgumentException("x: " + x + " or y: " + y + " out of range.");
		}
	}
	
	public void drawRowOfPixels(int row, Color3f[] colors) throws IllegalArgumentException {
		int yValue = Constants.NBOFVERTICALPIXELS-row;
		for (int j = 0; j < Constants.NBOFHORIZONTALPIXELS; j++) { 
			drawPixel(j, yValue, colors[j].x, colors[j].y, colors[j].z);
		}
	}
